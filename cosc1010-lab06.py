...
# Lab 06
# Lab Section: 15
# Sources, people worked with, help given to: Michael Salcidl, Emmanual
# your
# comments
# here
...
for random in random_string:
    if random not in alphabet_acur.keys():
        alphabet_acur[random]=1
    else:
        alphabet_acur[random]+=1

for alpha in sorted(alphabet_acur.keys()):
    print(alpha,":",alphabet_acur[alpha])
...
least_occurred = "a"
least_number=25000
for key in alphabet_acur.keys():
    if alphabet_acur[key]>most_number:
        most_occurred=key
        most_number=alphabet_acur[key]
    if alphabet_acur[key]<least_number:
        least_occurred=key
        least_number=alphabet_acur[key]
...
# Output what the percentage of the string each character is, again in alphabetical
for al in sorted(alphabet_acur.keys()):
    print(al,':',alphabet_acur[al]/25)